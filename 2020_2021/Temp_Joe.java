...
@TeleOp(name="Temp_Joe")
public class Temp_Joe extends LinearOpMode {
...
//        Rotation.setTargetPosition(0);
        ...
        while (opModeIsActive()) {
            ...
            // This section of code will stop and reset the encoders on the ARM (i.e. rotation) motor.
            if (buttonX1) {
                telemetry.addData("WARNING: You are about to reset the ARM encoder", " ");
                ...
                while(RESET_ARM == 0) {
                    ...
                    buttonY1 = gamepad1.y;
                    buttonA1 = gamepad1.a;
                    telemetry.addData("Waiting.  Push Y or X", " ");
                    telemetry.update();
                    if (buttonY1) {
                        telemetry.addData("Resetting ARM encoder", " ");
                        Rotation.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                        Rotation.setTargetPosition(0);
                        Current_Rotation_Position = Rotation.getCurrentPosition();
                        Rotation.setMode(DcMotor.RunMode.RUN_TO_POSITION);
                        telemetry.addData("New ARM encoder: ", Rotation.getCurrentPosition());
                        telemetry.update();
                        RESET_ARM=1;
                    }
                    if (buttonA1) {
            ...
            if (dpad_down1){
                ...
                Current_Rotation_Position = Rotation.getCurrentPosition();
                telemetry.addData("Starting Position", Current_Rotation_Position);
                Rotation.setTargetPosition(2200);
                Current_Rotation_Position = 2200;
                telemetry.addData("New Position", Current_Rotation_Position);
                telemetry.update();
            }
